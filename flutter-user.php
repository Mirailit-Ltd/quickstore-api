...
class FlutterUserController extends FlutterBaseController
{
    ...
    public function register_routes()
    {
        ...
        register_rest_route(
            ...
            array(
                array(
                    'methods' => 'POST',
                    'callback' => array($this, 'reset_password_woo'),
                    'permission_callback' => function () {
                        return parent::checkApiPermission();
                    }
                ),
        ...
        register_rest_route(
            ...
            array(
                array(
                    'methods' => 'POST',
                    'callback' => array($this, 'apple_login_v2'),
                    'permission_callback' => function () {
                        return parent::checkApiPermission();
                    }
                ),
    ...
    public function reset_password_woo()
    {
        $json = file_get_contents('php://input');
        $params = json_decode($json, TRUE);
        $usernameReq = $params["user_login"];
        ...
        // Trigger the default WooCommerce password reset email
        // WC()->mailer()->emails['WC_Email_Customer_Reset_Password']->trigger($user_email, $user_data, $key);

        // Send the WC_email Customer Reset Password
        $wc_emails = WC()->mailer()->get_emails();
        $wc_emails['WC_Email_Customer_Reset_Password']->trigger($user_data->user_login, $key);
    ...
    public function register_2()
    {
        ...
        $emailReq = $params["email"];
        $role = $params["role"];
        if (is_array($params) && in_array('dokan_enable_selling', $params)) {
            $dokan_enable_selling = $params['dokan_enable_selling'];
        }
        if (is_array($params) && in_array('wcfm_membership_application_status', $params)) {
    ...
    // Apple Create or get User
    function createSocialAccountApple($email, $name, $firstName, $lastName, $userName, $userIdentifier)
    {
        $email_exists = email_exists($email);
        if ($email_exists) {
            $user = get_user_by('email', $email);
        ...
        // Set _apple_identifier meta for user
        update_user_meta($user_id, '_apple_identifier', $userIdentifier);
    ...
    public function apple_login($request)
    {
        ...
        if (!isset($user_email)) {
            // Check if userIdentifier is set in a user meta _apple_identifier, if not then create  a new user and set the userIdentifier to the user meta
            if (isset($userIdentifier)) {
                // Check if userIdentifier is set in a user meta _apple_identifier, if not then create  a new user and set the userIdentifier to the user meta
                $args = array('meta_key' => '_apple_identifier', 'meta_value' => $userIdentifier);
                $search_users = get_users($args);
                if (empty($search_users)) {
                    // Return no user with the same apple identifier
                    // Keep the user identfier with meesage in error log
                    error_log("No user with the same apple identifier: " . $userIdentifier);

                    return parent::sendError("invalid_login", "No user with the same apple identifier", 400);
                }
                if (count($search_users) > 1) {
                    return parent::sendError("invalid_login", "Too many users with the same apple identifier", 400);
                }
                $user = $search_users[0];
                $cookie = generateCookieByUserId($user->ID);
                $response['wp_user_id'] = $user->ID;
        ...
        return $this->createSocialAccountApple($user_email, $display_name, "", "", $user_name, $userIdentifier);
    }

    //TODO: New
    // Apple Login from Updated MSTORE plugin
    public function apple_login_v2($request)
    {
        $json = file_get_contents('php://input');
        $params = json_decode($json, TRUE);
